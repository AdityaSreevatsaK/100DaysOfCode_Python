...
def check_flights(origin_city_code, destination_city_code, from_time, to_time):
    ...
    try:
        ...
        flight_data = FlightData(
            ...
            destination_city=data[route_string][ONE]['cityTo'],
            destination_airport=data[route_string][ONE]['flyTo'],
            out_date=data[route_string][ZERO]['local_departure'].split('T')[ZERO],
            return_date=data[route_string][2]['local_departure'].split('T')[ZERO],
            stop_overs=ONE,
            via_city=data[route_string][ZERO]['cityTo']
        )
        return flight_data
    else:
        flight_data = FlightData(
            ...
            out_date=data[route_string][ZERO]['local_departure'].split('T')[ZERO],
            return_date=data[route_string][ONE]['local_departure'].split('T')[ZERO]
        )
